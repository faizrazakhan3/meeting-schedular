...
function MeetingModal({
  ...
  const fetchParticipants =
    async (meetingId: number) => {
      ...
      try {
        ...
        const response =
          await fetch(
            `https://10.200.32.63:5000/api/meetings/${meetingId}/participants`,
            {
              headers: {
                Authorization:
                  `Bearer ${token}`,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        selectedMeeting
          ? `https://10.200.32.63:5000/api/meetings/${selectedMeeting.id}`
  ...
  const handleCancel = async () => {
    try {
      ...
      const response = await fetch(
        `https://10.200.32.63:5000/api/meetings/${selectedMeeting.id}/cancel`,
        {
          method: "PATCH",
          headers: {
  ...
  const searchUsers = async (
    ...
    try {
      ...
      const response =
        await fetch(
          `https://10.200.32.63:5000/api/meetings/search-users?search=${value}`,
          {
            headers: {
              Authorization:
                `Bearer ${token}`,
